...
class OCEANMDM(torch.nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.input_hint_block = HintBlock(self.data_rep, 5, self.latent_dim)
    ...
    def encode_text(self, raw_text):#编码文本
        ...
        max_text_len = 20 if self.dataset in ['humanml', 'kit','interx'] else None  # Specific hardcoding for humanml dataset
    ...
    def cmdm_forward(self, x, timesteps, y=None, weight=1.0):
        ...
        emb = self.c_embed_timestep(timesteps)  # [1, bs, d 512]

        # seq_mask = y['hint'].sum(-1) != 0

        personality = torch.stack([y['O'].unsqueeze(1).repeat(1, x.size(3)), 
                                   y['C'].unsqueeze(1).repeat(1, x.size(3)),
                                   y['E'].unsqueeze(1).repeat(1, x.size(3)),
                                   y['A'].unsqueeze(1).repeat(1, x.size(3)),
                                   y['N'].unsqueeze(1).repeat(1, x.size(3))], 2) #[bs,196,5]



        guided_hint = self.input_hint_block(personality.float())  # [seqlen,bs, d]相当于spatial encoder
    ...
    def forward(self, x, timesteps, y=None):
        ...
        timesteps: [batch_size] (int)
        """
        if 'O' in y.keys(): #如果有大五人格模型数值，则进入计算controlnet输出
